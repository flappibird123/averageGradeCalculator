...
function applyTheme(theme) {
    ...
    document.querySelectorAll('.criterion-box input').forEach(input => {
        if (input.classList.contains('high'))
            input.style.backgroundColor = theme.highColor;
        if (input.classList.contains('low'))
            input.style.backgroundColor = theme.lowColor;
    });
  ...
  `;
    ...
    if (existing)
        existing.replaceWith(avgStyle);
    else
        document.head.appendChild(avgStyle);
    ...
    gradeInputs.forEach(input => {
        ...
        if (!isNaN(val)) {
            ...
            if (val >= 7) {
                ...
                input.style.backgroundColor = (_a = getCurrentTheme()) === null || _a === void 0 ? void 0 : _a.highColor;
            }
            else if (val <= 2) {
                input.classList.add('low');
                input.style.backgroundColor = (_b = getCurrentTheme()) === null || _b === void 0 ? void 0 : _b.lowColor;
            }
            else {
...
copyBtn.addEventListener('click', () => {
    var _a;
    const match = (_a = resultDiv.textContent) === null || _a === void 0 ? void 0 : _a.match(/[\d.]+/);
    if (!match)
        return;
